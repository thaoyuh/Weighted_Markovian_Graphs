...
def plot_policy(n, mA, obstacle_mask, grid_positions, P, pi,
    ...
    for i in range(n * n):
        ...
        if obstacle_mask[i]:
            ...
            ax.add_patch(circle)
            if show_node_labels:
                ax.text(c, r, str(i), ha='center', va='center', fontsize=6, zorder=4)
...
def save_convergence_plots(iter_h, eff_h, kw_h, var_h, pierr_h,
    ...
    for vals, iv, ov, yl, tit, col, suf in [
        (var_h, m_init['Net_Var'], m_opt['Net_Var'], r'$V_{\mathcal{W}}$', 'Variance', 'purple', 'var'),
        (kw_h,  m_init['K_W'],    m_opt['K_W'],    r'$K_{\mathcal{W}}$', 'Mean',     'orange', 'mean'),
        (eff_h, m_init['Eff_Idx'],m_opt['Eff_Idx'],r'$\lambda$',        'Surprise',  'blue',   'eff'),
    ]:
...
def run_opt(mA, W, W2, pi_hat, obj, label, eta=1e-4, max_iter=8000,
            seed=42, a=0.005, verbose=True):
...
if __name__ == "__main__":
    ...
    print(f"Baseline: K_W={m_unif4['K_W']:.2f}, V_W={m_unif4['Net_Var']:.2f}, "
          f"lambda={m_unif4['Eff_Idx']:.2f}")

    # Min-Variance optimization
    res_1 = run_opt(mA4, W4, W2_4, pi4, 'minimize_variance',
                    '1: Min-Var (4x4)', eta=ETA_SMALL, max_iter=8000, seed=SEED)
    m_1 = res_1['mopt']
    print(f"  V_W: {m_unif4['Net_Var']:.2f} -> {m_1['Net_Var']:.2f}")
    ...
    res_2a = run_opt(mA4, W4, W2_4, pi4, 'maximize_efficiency',
                     '2a: Max-Surprise (4x4 Det)', eta=ETA_SMALL, max_iter=8000, seed=SEED+1)
    m_2a = res_2a['mopt']
    print(f"  lambda: {m_unif4['Eff_Idx']:.2f} -> {m_2a['Eff_Idx']:.2f}")
    ...
    res_2b = run_opt(mA4s, W4s, W2_4s, pi4s, 'maximize_efficiency',
                     '2b: Max-Surprise (4x4 Stoch)', eta=ETA_SMALL, max_iter=8000, seed=SEED+2)
    m_2b = res_2b['mopt']
    mask_e4s = mA4s > 0
    cvs4 = CV4s[mask_e4s]
    corr4 = np.corrcoef(res_2b['Popt'][mask_e4s], CV4s[mask_e4s])[0, 1]
    print(f"  lambda: {m_s4_unif['Eff_Idx']:.2f} -> {m_2b['Eff_Idx']:.2f}, Corr(P,CV)={corr4:.4f}")
    ...
    print(f"  Det baseline: K_W={m_d8_unif['K_W']:.2f}, V_W={m_d8_unif['Net_Var']:.2f}, "
          f"lambda={m_d8_unif['Eff_Idx']:.2f}, pi_err={m_d8_unif['pi_error']:.2e}")

    res_3a = run_opt(mA8, W8, W2_8d, pi8, 'maximize_efficiency',
                     '3a: Max-Surprise (8x8 Det)', eta=ETA_BIG,
                     max_iter=12000, seed=SEED+3, a=0.005)
    m_3a = res_3a['mopt']
    print(f"  lambda: {m_d8_unif['Eff_Idx']:.2f} -> {m_3a['Eff_Idx']:.2f}")
    ...
    mask_e8s = mA8s > 0
    cvs8 = CV8s[mask_e8s]
    print(f"  Stoch baseline: lambda={m_s8_unif['Eff_Idx']:.2f}, "
    ...
    res_3b = run_opt(mA8s, W8s, W2_8s, pi8s, 'maximize_efficiency',
                     ...
                     max_iter=12000, seed=SEED+4, a=0.005)
    m_3b = res_3b['mopt']
    P_opt_8s = res_3b['Popt']
    corr8 = np.corrcoef(P_opt_8s[mask_e8s], CV8s[mask_e8s])[0, 1]
    print(f"  lambda: {m_s8_unif['Eff_Idx']:.2f} -> {m_3b['Eff_Idx']:.2f}, Corr(P,CV)={corr8:.4f}")
    ...
    L.append("-" * 65)
    L.append(f"{'Policy':<20} {'K_W':>10} {'V_W':>14} {'lambda':>12} {'pi_err':>10}")
    L.append("-" * 65)
    for lbl, m in [('Uniform', m_unif4), ('Min-Variance', m_1), ('Max-Surprise', m_2a)]:
        L.append(f"{lbl:<20} {m['K_W']:>10.2f} {m['Net_Var']:>14.2f} "
                 f"{m['Eff_Idx']:>12.2f} {m['pi_error']:>10.1e}")
    var_red = (1 - m_1['Net_Var'] / m_unif4['Net_Var']) * 100
    lam_gain = m_2a['Eff_Idx'] / m_unif4['Eff_Idx']
    L.append(f"\nMin-Var: V_W reduced by {var_red:.1f}% (Hamiltonian cycle, {dom_edges} dominant edges)")
    L.append(f"Max-Surprise: lambda increased {lam_gain:.0f}x over uniform")
    ...
    for lbl, m in [('Uniform', m_s4_unif), ('Max-Surprise', m_2b)]:
        L.append(f"{lbl:<20} {m['K_W']:>10.2f} {m['Net_Var']:>14.2f} "
                 f"{m['Eff_Idx']:>12.2f} {m['pi_error']:>10.1e}")
    lam_s4 = m_2b['Eff_Idx'] / m_s4_unif['Eff_Idx']
    L.append(f"\nMax-Surprise: lambda increased {lam_s4:.0f}x, Corr(P,CV)={corr4:.4f}")
    ...
    for lbl, m in [('Uniform', m_d8_unif), ('Max-Surprise', m_3a)]:
        L.append(f"{lbl:<20} {m['K_W']:>10.2f} {m['Net_Var']:>14.2f} "
                 f"{m['Eff_Idx']:>12.2f} {m['pi_error']:>10.1e}")
    lam_8d = m_3a['Eff_Idx'] / m_d8_unif['Eff_Idx'] if m_d8_unif['Eff_Idx'] > 0 else float('inf')
    L.append(f"\nMax-Surprise: lambda increased {lam_8d:.0f}x over uniform")
    ...
    for lbl, m in [('Uniform', m_s8_unif), ('Max-Surprise', m_3b)]:
        L.append(f"{lbl:<20} {m['K_W']:>10.2f} {m['Net_Var']:>14.2f} "
                 f"{m['Eff_Idx']:>12.2f} {m['pi_error']:>10.1e}")
    lam_8s = m_3b['Eff_Idx'] / m_s8_unif['Eff_Idx'] if m_s8_unif['Eff_Idx'] > 0 else float('inf')
    L.append(f"\nMax-Surprise: lambda increased {lam_8s:.0f}x, Corr(P,CV)={corr8:.4f}")
